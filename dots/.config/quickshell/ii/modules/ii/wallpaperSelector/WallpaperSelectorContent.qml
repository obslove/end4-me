...
MouseArea {
    ...
    property var activeFilterField: null
    ...
    Keys.onPressed: event => {
        if (event.key === Qt.Key_Escape) {
            ...
            if (root.activeFilterField && root.activeFilterField.text.length > 0) {
                root.activeFilterField.text = root.activeFilterField.text.substring(0, root.activeFilterField.text.length - 1);
            }
            ...
            if (event.text.length > 0 && root.activeFilterField) {
                showControls = true;
                root.activeFilterField.text += event.text;
                root.activeFilterField.cursorPosition = root.activeFilterField.text.length;
                root.activeFilterField.forceActiveFocus();
            }
            event.accepted = true;
        }
    ...
    Rectangle {
        ...
        Item {
            ...
            Rectangle {
                anchors.fill: parent
                radius: Math.max(Appearance.rounding.none, wallpaperGridBackground.radius - 4)
                    ...
                    Row {
                        ...
                        Loader {
                            ...
                            sourceComponent: Toolbar {
                                ...
                                ToolbarTextField {
                                    id: filterField
                                    Component.onCompleted: root.activeFilterField = filterField
                        ...
                        Loader {
                            ...
                            sourceComponent: Toolbar {
                                ToolbarTextField {
                                    id: onlineFilterField
                                    Component.onCompleted: root.activeFilterField = onlineFilterField
                                    Component.onDestruction: if (root.activeFilterField === onlineFilterField) root.activeFilterField = null
